...
class NBA:
    def __init__(self, gnba):
        ...
        if self.final_number > 0:
            ...
            for gnba_node_idx in range(len(gnba.nodes)):
                gnba_node = self.gnba.nodes[gnba_node_idx]
                nba_node = NBA_node(gnba_node, -1, gnba_node_idx)
                for next_idx in gnba_node.next:
                    nba_node.add_next(-1, next_idx)
                self.nodes[(-1, gnba_node_idx)] = nba_node
            for idx in self.gnba.initial:
                self.initial.append((-1, idx))